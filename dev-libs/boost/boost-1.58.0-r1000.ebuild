...
src_prepare() {
	epatch \
		...
		"${FILESDIR}/${PN}-1.56.0-mpi_python_c++11.patch" \
		"${FILESDIR}/${PN}-1.56.0-build_auto_index.patch" \
		"${FILESDIR}/${PN}-1.58.0-fusion_non-constexpr.patch" \
		"${FILESDIR}/${PN}-1.58.0-python-3.3.patch" \
		"${FILESDIR}/${PN}-1.58.0-python-3.1.patch"

	# https://svn.boost.org/trac/boost/ticket/10507
	sed -e "/wave\/build\/\/wave/d" -i tools/Jamfile.v2 || die "sed failed"
